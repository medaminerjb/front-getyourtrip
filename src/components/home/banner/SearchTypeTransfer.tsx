...
import "./searchTypeTransfer.css"; // custom styling (below)
import React, { useEffect, useRef, useState } from "react";
interface TransferType {
checked:string;
}
interface GuestBoxProps{
OnCheck?: (checked: TransferType) => void;
}
const SearchTypeTransfer: React.FC<GuestBoxProps> = ({ OnCheck }) => {
  const [activeTab, setActiveTab] = useState(false);
  const [selected, setSelected] = useState<string>("Only Arrival");
...
const dropdownRef = useRef<HTMLDivElement>(null);
    ...
    OnCheck?.({ checked: value });
                ...
                const options = [
    { label: "Only Departure", value: "departure" },
    { label: "Only Arrival", value: "arrival" },
    { label: "Arrival and Departure", value: "arrivalAndDeparture" },
  ];
                    ...
                    <div className={`searchbox-input ${activeTab ? "active" : "" }`} ref={dropdownRef}>
                        ...
                        <div className="custom-select-dropdown style-2">
                            <div className="select-input" onClick={toggleDropdown}>
                                <h6 ><span  id="transfer-type">{selected}</span></h6><i className="bi bi-chevron-down"></i>
                            </div>
                            <div className={`custom-select-wrap two no-scroll ${activeTab ? "active" : "" }`}>
                                <ul className="option-list">
                                  <div className="checkbox-container">
																			   {options.map((opt) => (
                  <label key={opt.value} className="check-container">
                    {opt.label}
                    <input
                      type="checkbox"
                ...
                export default SearchTypeTransfer;