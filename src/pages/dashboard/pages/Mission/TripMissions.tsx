...
import ActivityTripModel from 'sub-components/mission/ActivityTripModel';
...
const MissionsTrip: React.FC = () => {
  ...
  const [showAddActivityTrip,setShowAddActivityTrip]=useState<boolean>(false);
 ...
 pk: 0,
  ...
  const fetchMissions = useCallback(async (page: number = 1, search: string = '') => {
    try {
      ...
      const response = await axios.get(`${apiurl}/missions/trip`, {
        params: { page, search },
          headers: {
    ...
    try {
      ...
      const payload = {
        ...
        tenant:user.tenant
             ...
             const apiurl = import.meta.env.VITE_API_URL;
      const url = editingMission ? `${apiurl}/missions/trip/` : `${apiurl}/missions/trip/`;
...
<a className="btn btn-ghost btn-icon btn-sm rounded-circle texttooltip">
              ...
              <div className="modal-body">
                <form onSubmit={submitMissionForm}>
                  <input type="hidden" name="id" value={editingMission?.pk || ''} />
                      ...
                      <a href="#!" className="btn-link fw-semi-bold" onClick={()=>setShowAddActivityTrip(true)}>
                      Add New
                      </a>
  ...
  <button 
      ...
      {showAddActivityTrip && (
        <ActivityTripModel
        show={showAddActivityTrip}
        handleClose={()=>setShowAddActivityTrip(false)}
